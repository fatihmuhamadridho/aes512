...
class Aes {
  ...
  static cipher(input, w) {
    const Nb = 8; // block size (in words): no of columns in state (fixed at 4 for AES)
    const Nr = 22; // no of rounds: 10/12/14 for 128/192/256-bit keys
  ...
  static keyExpansion(key) {
    const Nb = 8; // block size (in words): 8 columns in state (fixed at 8 for AES-512)
    const Nk = key.length / 8; // key length (in words): 8 words for 512-bit key
    const Nr = Nk + 6; // no of rounds: 14 for 512-bit key
